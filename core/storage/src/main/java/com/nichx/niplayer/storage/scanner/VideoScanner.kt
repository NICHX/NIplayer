 ...
 *
 * 来源包括系统 MediaStore 与用户扩展目录（File 递归遍历），
 * 通过 `VideoStorage.deepRefresh` 做合并去重 + 增量同步。
 *
 * 数据来源：
 ...
 * （`isExtend=false`），与 `distinctBy` 取最后一个的行为一致。
 *
...
class VideoScanner @Inject constructor(
     ...
     * 递归遍历文件夹，收集视频文件。
     *
     * 用 [VideoExtensionSettings.isVideoFile] 判断
