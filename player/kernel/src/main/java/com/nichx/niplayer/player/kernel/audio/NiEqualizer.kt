 ...
 * - [attach]：在获取到有效 audioSessionId 后创建 Equalizer 并应用传入的 [EqualizerConfig]
 * - [release]：释放 Equalizer 资源（播放器销毁 / audioSessionId 变化时调用）
 * - [applySettings]：按传入的 [EqualizerConfig] 重新应用配置（UI 修改设置后调用）
 *
 ...
 *
 * **A1 架构修复（2026-09-21）**：本类原先直接读取 :core:datastore 的全局 `AudioSettings`，
 * 使播放内核依赖设置持久化层。现改为**由调用方传入** [EqualizerConfig]，读取职责上移到
 * 依赖设置层的模块（见 [EqualizerConfigProvider]）。全部防爆音语义保持不变。
 */
class NiEqualizer {
    ...
    /**
     * 绑定到指定 audioSessionId，创建 Equalizer 并应用 [config]。
     *
     * 当均衡器关闭（[EqualizerConfig.enabled] = false）时不创建 Equalizer 实例：
     ...
     * @param config 本次要应用的均衡器配置
     */
    fun attach(audioSessionId: Int, config: EqualizerConfig) {
    ...
    /**
     * 按 [config] 重新应用配置到当前 Equalizer。
     *
     * 调用前必须已 [attach]。UI 修改设置后调用此方法实时生效。
     *
     * 关闭均衡器（[EqualizerConfig.enabled] = false）时保持效果 enabled，
     ...
     */
    fun applySettings(config: EqualizerConfig) {
    ...
    /** 按 [config] 写入 [eq] 的频段参数（不改变 enabled 状态）。 */
    private fun applyTo(eq: Equalizer, config: EqualizerConfig) {
        if (config.enabled) {
            val presetIndex = config.presetIndex
            if (presetIndex >= 0 && presetIndex < eq.numberOfPresets) {
                eq.usePreset(presetIndex.toShort())
            } else {
                // 自定义模式：逐 band 设置增益
                for (band in 0 until eq.numberOfBands.toInt()) {
                    val level = config.bandLevel(band)
