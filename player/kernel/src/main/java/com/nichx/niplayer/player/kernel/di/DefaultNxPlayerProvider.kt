 ...
 *    - 非 "auto"：**显式强制指定内核**，跳过能力过滤（用户选择优先权高于能力探测），
 *      无效 id 时回退全部；
 *    - "auto"：保留全部，走能力过滤。
 * 2. 自动模式按 [NxMediaSource] 能力过滤（source 为 null 时跳过）。
 * 3. 按 [NxPlayerBackend.backendPriority] 降序排序。
 * 4. 若结果为空，回退全部已注册内核（media3 恒兜底，保证非空）。
 */
@Singleton
class DefaultNxPlayerProvider @Inject constructor(
    ...
    override fun resolveBackends(source: NxMediaSource?): List<NxPlayerBackend> {
        // 1. 设置覆盖：显式强制指定内核（跳过能力过滤），或自动选全部
        val forced = PlayerSettings.playerBackend
        val forcedMode = forced != NX_BACKEND_AUTO
        val basePool: Collection<NxPlayerBackend> =
            if (forcedMode) backends.filter { it.backendId == forced }
            else backends
        val pool = basePool.ifEmpty { backends }

        // 2. 能力过滤：显式强制或 source 为 null 时跳过
        val capable = if (source == null || forcedMode) pool.toList() else pool.filter { it.supports(source) }

        // 3. 优先级降序
        val ordered = capable.sortedByDescending { it.backendPriority }

        // 4. 结果为空时回退全部，保证解析非空（media3 兜底）
