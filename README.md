...
> **一站式媒体播放体验** —— 统一访问本地 / SAF / SMB / WebDAV 四类存储，轻松播放视频、音频与图片，内置专业级播放控制、ASS 特效字幕引擎、歌单系统与 WebDAV 云同步，支持多语言与应用内在线更新。完全重构自 [NIplayer v1.x](https://github.com/nichx/NIplayer)。
...
- **自定义歌单**：创建 / 重命名 / 删除，随心整理收藏
- **批量添加**：媒体库与搜索结果中一键收藏到歌单
- **歌单置顶**：常用歌单置顶排序，快速触达
- **歌单连播**：歌单内顺序播放，退出后自动记忆进度并恢复歌单列表
...
- **数据备份**：数据库（媒体库 / 历史 / 歌单 / 书签 / 设置）JSON 一键导出 / 导入，换机迁移
