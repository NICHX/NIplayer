...
- **断点续播**：再次打开弹窗"接着上次看"
- **控制栏自定义**：左右栏按钮增删、排序、所在侧与位置可配置
- **竖屏竖滑切视频**：竖屏向下 / 向上滑在相邻视频间快速切换（可关闭，竖滑不影响亮度 / 音量手势）
- **媒体信息**：编码 / 分辨率 / 码率 / 帧率 / HDR 格式，HDR 缩略图优化
- **锁屏防误触**：画面锁定后隐藏控制栏、禁用手势

**独立窗口转场**：视频播放器由独立 Activity 承载，进入 / 退出时播放画面随窗口一起右下角滑入 / 滑出，过渡流畅且与 UI 同步。
...
- **远程文件管理**：SMB / WebDAV 支持重命名、移动、复制、新建文件夹、删除
- **文件浏览**：目录导航、排序、列表 / 网格视图、媒体类型过滤、递归搜索；修改 / 新建 / 密码等输入弹窗自动拉起键盘，返回手势优先关闭底部弹层 / 文件菜单
- **存储源网格视图**：媒体库存储源支持列表 / 网格视图切换，浏览更直观
- **多选批量操作**：长按进入多选，全选 / 批量下载 / 批量删除 / 批量移动 / 批量复制 / 批量添加到快速访问；低频操作收纳进「更多」菜单保持操作栏简洁
- **目录浏览式目标选择器**：移动 / 复制目标采用逐级下钻的目录浏览器，点子目录进入浏览、点「移动到此处 / 复制到此处」才真正执行，避免误触即操作
- **本地扫描**：MediaStore + 扩展目录合并，增量去重；扫描任务可视化管理
- **播放历史**：按日期 / 文件夹分组，视频 / 音频筛选，点击续播
- **快速访问**：书签式收藏，长按拖拽排序
...
- **下载管理**：多任务并发、断点续传、批量下载，实时进度 / 速度 / 剩余时间；下载设置弹窗统一管理目录与「顺带下载同目录歌词」开关
- **应用内更新**：启动自动检查新版本（24 小时节流、静默失败），检测到更新后一键下载安装，支持后台下载

## 🎨 个性化

- **主题**：浅色 / 暗色 / 跟随系统，多套预设配色方案一键切换
- **多语言**：应用内切换简体中文 / English，无需跟随系统
- **播放统计**：观看次数 / 时长、媒体类型与存储分布、Top 观看榜单
- **视频书签**：时间轴打点标记，快速跳转
- **封面控制**：可选退出播放后更新封面，缩略图生成更省资源
- **短视频出图**：时长 < 15 秒的视频与本地视频均取帧生成缩略图，不再显示占位图标；取帧位置可选「5 秒 / 10% / 50%」
- **服务端缩略图维护**：删除视频时清理应用上传的缩略图，浏览时惰性清理孤立略图；移动文件删除旧图、重命名同步改名，保持服务端 `.thumb/` 目录整洁
...
- **弹窗玻璃化**：下载目录选择、复制 / 移动目录浏览等宽面板弹窗统一采用磨砂面板 + 玻璃细描边，全应用视觉一致
